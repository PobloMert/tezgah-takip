...
class TezgahTakipMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Managers - Enhanced Database Manager ile güncellenmiş
        try:
            self.config_manager = ConfigManager()
            
            # Enhanced Database Manager kullan
            ...
            # Veritabanı durumunu kontrol et ve kullanıcıya bildir
            db_status = self.enhanced_db_manager.get_database_status()
            if db_status.is_fallback and db_status.fallback_type:
                self.logger.warning(f"⚠️ Veritabanı fallback modunda: {db_status.fallback_type.value}")
                # Ana pencere açıldıktan sonra kullanıcıya bildirim gösterilecek
                self._show_fallback_notification = True
                self._fallback_type = db_status.fallback_type
            else:
                self._show_fallback_notification = False
        ...
        # UI kurulumu
        self.setWindowTitle("🏭 TezgahTakip - AI Güçlü Fabrika Bakım Yönetim Sistemi v2.1.3")
    ...
    def showEvent(self, event):
        """Pencere gösterildiğinde çağrılır - Fallback bildirimi için"""
        super().showEvent(event)
        
        # Fallback bildirimi göster (eğer gerekiyorsa)
        self._show_fallback_notification_if_needed()
    ...
    def cleanup_resources(self):
        ...
        try:
            ...
            # Enhanced Database Manager'ı temizle
            if hasattr(self, 'enhanced_db_manager') and self.enhanced_db_manager:
                try:
                    # Performans metriklerini logla
                    metrics = self.enhanced_db_manager.get_connection_health_report()
                    self.logger.info(f"Database performance metrics: {metrics}")
                    
                    # Bağlantıyı kapat
                    self.enhanced_db_manager.close()
                    self.logger.info("Enhanced database connection closed")
                except Exception as db_error:
                    self.logger.error(f"Enhanced database cleanup error: {db_error}")
            
            # Backward compatibility için normal db_manager'ı da temizle
    ...
    def _handle_initialization_error(self, error: Exception):
        """Başlatma hatalarını işle - Enhanced error handling"""
        try:
            from enhanced_database_manager import EnhancedDatabaseManager
            from exception_handler import EnhancedErrorHandler
            
            # Enhanced error handler kullan
            error_handler = EnhancedErrorHandler()
            error_analysis = error_handler.analyze_sqlite_error(error)
            user_message = error_handler.format_user_message(error, error_analysis.suggested_actions)
            
            self.logger.error(f"❌ Ana pencere başlatma hatası: {user_message}")
            
            # Kullanıcı dostu hata mesajı
            detailed_message = (
                f"🏭 TezgahTakip Başlatma Hatası\n\n"
                f"{user_message}\n\n"
                f"💡 Önerilen çözümler:\n"
            )
            ...
            detailed_message += f"\n🔧 Teknik detay: {str(error)}"
            
            # Hata dialog'unu göster (eğer mümkünse)
            try:
                CustomMessageBox.critical(None, "Başlatma Hatası", detailed_message)
            except:
                # Fallback - console'a yazdır
                print(f"CRITICAL ERROR: {detailed_message}")
    ...
    def _show_fallback_notification_if_needed(self):
        """Gerekirse fallback bildirimi göster"""
        try:
            if hasattr(self, '_show_fallback_notification') and self._show_fallback_notification:
                fallback_type = getattr(self, '_fallback_type', None)
                
                if fallback_type:
                    from database_error_models import FallbackType
                    
                    # Fallback türüne göre mesaj
                    if fallback_type == FallbackType.MEMORY_DATABASE:
                        title = "🧠 Bellek Veritabanı Aktif"
                        message = (
                            "Veritabanı geçici olarak bellekte çalışıyor.\n\n"
                            "⚠️ Önemli: Verileriniz kalıcı değil!\n"
                            "• Uygulama kapandığında veriler kaybolacak\n"
                            "• Lütfen veritabanı sorununu çözün\n"
                            "• Yedeklerinizi kontrol edin\n\n"
                            "💡 Çözüm için 'Ayarlar > Veritabanı' menüsünü kontrol edin."
                        )
                    elif fallback_type == FallbackType.BACKUP_RESTORE:
                        title = "📦 Yedek Veritabanı Yüklendi"
                        message = (
                            "Ana veritabanı erişilemediği için yedek kullanılıyor.\n\n"
                            "✅ Verileriniz güvende\n"
                            "• Son yedekten geri yüklendi\n"
                            "• Normal şekilde çalışmaya devam edebilirsiniz\n\n"
                            "💡 Ana veritabanı sorunu çözüldüğünde otomatik geçiş yapılacak."
                        )
                    elif fallback_type == FallbackType.CLEAN_DATABASE:
                        title = "🆕 Yeni Veritabanı Oluşturuldu"
                        message = (
                            "Mevcut veritabanı kullanılamadığı için yeni bir tane oluşturuldu.\n\n"
                            "ℹ️ Bilgi:\n"
                            "• Temiz bir veritabanı ile başlıyorsunuz\n"
                            "• Eski verileriniz kaybolmuş olabilir\n"
                            "• Yedeklerinizi kontrol edin\n\n"
                            "💡 Veri kurtarma için 'Yardım > Destek' ile iletişime geçin."
                        )
                    else:
                        title = "🔄 Yedek Sistem Aktif"
                        message = (
                            "Veritabanı yedek sistemle çalışıyor.\n\n"
                            "⚠️ Dikkat: Normal performanstan farklılık olabilir\n"
                            "• Sistem otomatik olarak düzelmeye çalışacak\n"
                            "• Verileriniz korunmaya devam ediyor\n\n"
                            "💡 Sorun devam ederse teknik destek ile iletişime geçin."
                        )
                    
                    # Bildirim göster (5 saniye sonra)
                    QTimer.singleShot(5000, lambda: CustomMessageBox.information(self, title, message))
                
                # Bildirimi sadece bir kez göster
                self._show_fallback_notification = False
                
        except Exception as e:
            self.logger.error(f"Fallback notification error: {e}")
    
    def show_database_status(self):
        """Veritabanı durumunu göster - Enhanced"""
        try:
            if hasattr(self, 'enhanced_db_manager'):
                # Enhanced database manager durumu
                status = self.enhanced_db_manager.get_database_status()
                health_report = self.enhanced_db_manager.get_connection_health_report()
                fallback_metrics = self.enhanced_db_manager.get_fallback_usage_metrics()
                
                status_text = f"""
🗄️ VERİTABANI DURUM RAPORU
{'='*50}

📊 Bağlantı Durumu:
   • Durum: {'🟢 Bağlı' if status.is_connected else '🔴 Bağlantısız'}
   • Yol: {status.database_path}
   • Fallback: {'🟡 Aktif' if status.is_fallback else '🟢 Normal'}
   • Fallback Türü: {status.fallback_type.value if status.fallback_type else 'Yok'}

🔍 Bütünlük Durumu:
   • Durum: {status.integrity_status}
   • Son Kontrol: {status.last_check_time.strftime('%d.%m.%Y %H:%M:%S')}
   • Bağlantı Denemeleri: {status.connection_attempts}

📈 Performans Metrikleri:
   • Toplam Bağlantı: {health_report.get('total_connections', 0)}
   • Başarılı: {health_report.get('successful_connections', 0)}
   • Başarısız: {health_report.get('failed_connections', 0)}
   • Ortalama Süre: {health_report.get('average_connection_time', 0):.3f}s
   • Son Bağlantı: {health_report.get('last_connection_time', 'Yok')}

🔄 Fallback İstatistikleri:
   • Toplam Aktivasyon: {fallback_metrics.get('total_activations', 0)}
   • Bellek DB: {fallback_metrics.get('memory_database_usage', 0)}
   • Yedek Geri Yükleme: {fallback_metrics.get('backup_restore_usage', 0)}
   • Temiz DB: {fallback_metrics.get('clean_database_usage', 0)}

⚠️ Son Hata: {status.last_error or 'Yok'}
"""
            else:
                # Fallback - normal database manager
                status_text = f"""
🗄️ VERİTABANI DURUM RAPORU (Basit Mod)
{'='*50}

📊 Bağlantı Durumu:
   • Durum: 🟢 Bağlı (Basit Mod)
   • Yol: {getattr(self.db_manager, 'db_path', 'Bilinmiyor')}
   • Tezgah Sayısı: {self.db_manager.get_tezgah_count() if hasattr(self.db_manager, 'get_tezgah_count') else 'Bilinmiyor'}

⚠️ Enhanced Database Manager aktif değil.
   Gelişmiş özellikler için sistem yeniden başlatılmalı.
"""
            
            CustomMessageBox.information(self, "Veritabanı Durumu", status_text)
            
        except Exception as e:
            self.logger.error(f"Database status display error: {e}")
            CustomMessageBox.critical(self, "Hata", f"Veritabanı durumu gösterilirken hata oluştu:\n{e}")
...
"""
    ...
    def create_ayarlar_tab(self):
        ...
        version_label = QLabel("TezgahTakip v2.1.3")
