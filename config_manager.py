...
class ConfigManager:
    ...
    DEFAULT_CONFIG = {
        "database": {
            ...
            "fallback_paths": [
                "tezgah_takip_v2.db",
                "backups/tezgah_takip_v2.db",
                "%USERPROFILE%/Documents/TezgahTakip/tezgah_takip_v2.db",
                "%APPDATA%/TezgahTakip/tezgah_takip_v2.db",
                "C:/TezgahTakip/tezgah_takip_v2.db"
            ],
    ...
    def get_database_fallback_paths(self) -> List[str]:
        """Veritabanı fallback yollarını al"""
        fallback_paths = self.get("database.fallback_paths", [])
        
        # Environment variable'ları genişlet
        expanded_paths = []
        for path in fallback_paths:
            try:
                ...
                self.logger.warning(f"Path expansion failed for {path}: {e}")
                expanded_paths.append(path)
        
        return expanded_paths
    
    def update_database_path(self, new_path: str) -> bool:
        ...
        try:
            ...
            self.set("database.path", normalized_path)
            
            # Kaydet
            return self.save_config()
            
        except Exception as e:
            self.logger.error(f"Database path update error: {e}")
            return False
    
    def add_fallback_path(self, path: str) -> bool:
        """Yeni fallback yolu ekle"""
        try:
            current_paths = self.get_database_fallback_paths()
            ...
            if normalized_path not in current_paths:
                ...
                self.set("database.fallback_paths", current_paths)
                return self.save_config()
            
            return True
            
        except Exception as e:
            self.logger.error(f"Add fallback path error: {e}")
            return False
    
    def is_database_fallback_enabled(self) -> bool:
        """Veritabanı fallback sistemi etkin mi"""
        return self.get("database.enable_fallback", True)
    
    def is_database_integrity_check_enabled(self) -> bool:
        """Veritabanı bütünlük kontrolü etkin mi"""
        return self.get("database.integrity_check_enabled", True)
    
    def get_database_performance_settings(self) -> Dict:
        ...
        return {
            ...
            "performance_monitoring": self.get("database.performance_monitoring", True),
            "integrity_check_interval_hours": self.get("database.integrity_check_interval_hours", 24),
            "auto_repair_enabled": self.get("database.auto_repair_enabled", True)
        }
